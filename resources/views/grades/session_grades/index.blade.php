...
@section('title', 'Годишни оценки')

@section('heading1', 'Годишни оценки')
                ...
                <tr>
                    ...
                    <td class="grades">
                        <div class="grade">
                            @if(Auth::guard('administrator')->check())
                                ...
                                @include('grades.context_menu', ['grades_page' => 'session_grades', 'id' => $random_id])
                            @else
                    ...
                    <td>
                        @if(Auth::guard('teacher')->check() && \App\Models\Subject::isOwn(Session::get('workingSubjectId')))
                            @if(Session::get('workingClassNumber') == $grade->for_class)
                                @if(\App\Models\Term::getLastTerm()->now)
                                    <a href="{{route('session_grades.form', $grade->student_egn)}}"><i class="grade-add-btn fas fa-plus"></i></a>
                                @endif
                            @endif
                        @endif
                    </td>
